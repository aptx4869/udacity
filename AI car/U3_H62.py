...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        steer_angle = float(motion[0]) + random.gauss(0.0,self.steering_noise)
        ...
        if float(motion[0]) > max_steering_angle:
            raise ValueError, 'steering angle must be in [0..pi / 4]'

        beta = distance / self.length * tan(steer_angle)
        ...
        if abs(beta) > 0.001:
            ...
            x=self.x + distance * cos(orientation)
            y=self.y + distance * sin(orientation)
        orientation = (orientation + beta)
        orientation %= 2 * pi

        result = robot(self.length)
        result.set(x, y, orientation)
        result.set_noise(self.steering_noise, self.bearing_noise, self.distance_noise)
    ...
    # sense: 
    #    
    def sense(self,flag=None): #do not change the name of this function
        Z = []
        for i in range(len(landmarks)):
            beta = atan2(landmarks[i][0] - self.y , landmarks[i][1] - self.x) - self.orientation
            if flag is not None:
