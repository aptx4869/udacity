...
def query():
    sequence = db.execute("select * from links where submitter_id=62443 and votes > 1000")
    for link_tuple in sequence:
        link = Link(*link_tuple)
        return link.id
    #link = Link(*sequence.fetchmany())
    ##link=Link(tuple(sequence.fetchmany()))
    print link
