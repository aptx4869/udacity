...
#  
#     File: test.py
#     Create Date: 2012-05-25 09:18:51
#-------------------------------------------------
...
def readwordlist(filename):
    ...
    for pre in prefixset:
        ...
        for c in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ':
            if pre + c in wordset or pre + c in prefixset:
                result.append(c)
        nextchardic[pre]=frozenset(result)
        #print nextchardic[pre]

    return wordset, prefixset, nextchardic

WORDS, PREFIXES ,NEXTCHAR= readwordlist('words4k.txt')
...
ANY = anchor(LETTERS) # | anchor(list['_']) # The anchor that can be any letter
...
def add_suffixes(hand, pre, start, row, results, anchored=True):
    ...
    >>> add_suffixes('K','JAC',1,row,set([]),False)
    set([(1, 'JACK')])
    >>> add_suffixes('_','JAC',1,row,set([]),False)
    set([(1, 'JACk')])
    >>> len(add_suffixes('__','',1,row,set([]),False))
    24

    #>>> row_plays('_KCEHKN', row)
    """
    #if anchored:
        #print hand,pre, start,  results
    i = start + len(pre)
    PRE = pre.upper()
    #print is_letter(row[i])
    ...
    if PRE in PREFIXES:       
        ...
        if is_letter(sq):
            ...
            for L in hand:
                #if PRE == 'K' and start == 9:
                    #print hand,pre,start,L
                    #print possibilities
                if L == '_':
                    for l in NEXTCHAR[PRE]:
                        if l in possibilities:
                            add_suffixes(removed(hand,L), pre+l.lower(), start, row, results)
                else:
                    if L in possibilities:
...
def find_prefixes(hand, pre='', results=None):
    """find the possible prefixes and up to pre with chars in hand
    >>> find_prefixes('ABCDYOR', 'TOM')
    set(['TOMO', 'TOMOR', 'TOM'])
    >>> find_prefixes('ABCDY_R', 'TOM')
    set(['TOMo', 'TOMoR', 'TOM'])
    >>> find_prefixes('ABCD__R', 'TOM')
    set(['TOM', 'TOMor', 'TOMoR', 'TOMoRr', 'TOMo', 'TOMorR'])
    >>> find_prefixes('_Z', '')
    set(['', 'Za', 'Zw', 'aZ', 'Zy', 'Zo', 'Z', 'Zi', 'a', 'c', 'b', 'e', 'd', 'g', 'f', 'i', 'h', 'k', 'j', 'm', 'l', 'o', 'n', 'q', 'p', 's', 'r', 'u', 't', 'w', 'v', 'y', 'x', 'z', 'Ze'])
    """
    ...
    if PRE in PREFIXES:
       for L in hand:
            if L == '_':
                for l in NEXTCHAR[PRE]:
                    find_prefixes(hand.replace(L, '',1), pre+l.lower(),results)
            else:
...
def row_plays(hand, row):
    ...
    >>> row_plays('ACK', row)
    set([(1, 'JACK'), (6, 'KA'), (8, 'KA'), (13, 'AI')])

    >>> r = row_plays('_CK', row)
    >>> (1, 'JaCK') in r and (8, 'Ka') in r
    True
    >>> (6, 'Ka') in r and (13, 'aI') in r
    True
    >>> (9, 'KiCK') in row_plays('_KCEHKN', row)
    True
    """
...
def show(board):
    for i,L in enumerate(board):
        for j,c in enumerate(L):
            if BONUS[i][j] in (DW,TW,DL,TL) and c == '.':
                print BONUS[i][j],
            else:
                print c,
        print
...
#result = best_play('_BCEHKN', a_board())
#result = all_plays('_BCEHKN', a_board())
#result = best_play('_KCEHKN', a_board())
#print result
