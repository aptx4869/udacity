...
def best_play(hand, board):
    ...
    def word_score(play):
        (score,position,direction,word) = play
        return play
    results = all_plays(hand, board)
    return max(results,key=word_score)
    ###Your code here.
def a_board():
    ...
    for i,L in enumerate(board):
        for j,c in enumerate(L):
            if BONUS[i][j] in (DW,TW,DL,TL) and c == '.':
                ...
                print c,
        print
horizontal_plays('ABCDEF', test_boart)
show(test_boart)
print best_play('ABCDEF', test_boart)
#def calculate_score(board, pos, direction, hand, word):
