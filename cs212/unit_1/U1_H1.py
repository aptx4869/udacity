...
def best_hand(hand,n=7):
    "From a 7-card hand, return the best 5 card hand."
    lists=list(itertools.permutations(hand, 5))
    return max(lists,key=hand_rank)
    #t=[]
    #for key in lists:
        #v=list(key)
        #v.sort(key=one_card_rank)
        #try:
            #if t.index(v):
                #continue
        #except:
            #t.append(v)
    #return max(t,key=hand_rank)

    
def allmax(iterable, key=None):
    "Return a list of all items equal to the max of the iterable."
    # Your code here.
    iterable.sort(reverse = True, key = key)
    for i in range(len(iterable)):
        if key(iterable[i]) != key(iterable[0]):
            break
    return iterable[:i]

def one_card_rank(card):
