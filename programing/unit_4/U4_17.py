...
def path_cost(path):
    ...
    if len(path) < 3:
        ...
        return path[-2][-1] if len(path) % 2 else path[-1][-1]
        # ???
...
def test():
    print  path_cost(('fake_state1', ((2, 5, '->'), 5), 'fake_state2')) == 5
    print  path_cost(('fs1', ((2, 1, '->'), 2), 'fs2', ((3, 4, '<-'), 6), 'fs3')) == 6
    print  path_cost(('fs1', ((2, 1, '->'), 2), 'fs2', ((3, 4, '<-'), 6))) == 6
    assert bcost((4, 2, '->'),) == 4
